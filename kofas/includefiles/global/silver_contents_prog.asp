...
uploadform.defaultpath = server.mappath("\datafiles\")&"\sub06\"
uploadform.MaxFileLen			= 1024*1024*5
	...
	health			= uploadform.Form("health")
	health_ment		= uploadform.Form("health_ment")
	home			= uploadform.Form("home")
	home_ment		= uploadform.Form("home_ment")
	meal			= uploadform.Form("meal")
	meal_ment		= uploadform.Form("meal_ment")
	tar_content		= uploadform.Form("tar_content")
	tar_ask			= uploadform.Form("tar_ask")
...
If arng = "write" Then
	sql="select count(*) from [silver_contents] where group_seq = "&group_seq&" and sDate='"&wdate&"'"
	Cnt =	SqlValue(Sql,1)
	If cnt > 0 Then
		Call EndGoURL("활동일지가 이미 등록되어 있습니다.")
	End if
	set rs=server.createobject("adodb.recordset")
	rs.open "silver_contents", Sql_connect(), adopenstatic, adlockpessimistic,adcmdtable
ElseIf arng = "edit" Or arng = "del" Then
	sql="select filename1, filename2, filename3 from [silver_contents] where seq="&seq
...
Select Case arng
	Case "write"
			with rs
				...
				.fields("health_ment")		= health_ment
				.fields("home")				= home
				.fields("home_ment")		= home_ment
				.fields("meal")				= meal
				.fields("meal_ment")		= meal_ment
				.fields("tar_content")		= tar_content
				.fields("tar_ask")			= tar_ask
				.update
				.close
			end with
		If mode <> "admin" Then
 ...
 Case "edit"
			set rs=server.createobject("adodb.recordset")
			sql="select * from silver_contents where seq="&seq
			rs.open sql, Sql_connect(), adopenstatic, adlockpessimistic,adcmdtext
			with rs
				.fields("start_time")		= start_time
				.fields("end_time")			= end_time
		...
		End if
 Case "delete"
		Sql = "delete from silver_contents where Seq="&seq
		Sql = Sql &"; update GROUP_MEMBER_attendance set ["&sDay&"] = NULL  where id = '"&id&"' and group_seq="&group_seq&" and sYear="&sYear&" and sMOnth="&Int(sMonth)
		'Response.write Sql
		Call RunSql(sql, 1)
 End Select
set rs=Nothing
Call CloseDB
If mode = "admin" Then
	response.redirect "/administrator/silver/gr_contents_list.asp?page="&page&"&GR_INFO="&GR_INFO
else
