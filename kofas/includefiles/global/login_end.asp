...
Else
	...
	strSql = "select UserId, UserPw, UserLevel, seq, intranet, isnull(memgubun, '') from tbl_members "
...
'				RWbr("LogPW =>" & LogPW)
			...
			If MEMBERKEY = "" Then
				If arrFlds(1,0) <> LogPW And arrFlds(1,0) <>  getRequest("LogPW", REQUEST_POST) Then
					Call EndGoURL("이용 권한이 없습니다.")
				Else
					'비밀번호 암호화 한담
					pwdchk			= arrFlds(5,0)
					If pwdchk = "" Then
						strSql = "Update tbl_members set UserPw='"&LogPW&"', memgubun='Y' where UserId='"&LCase(Replace(LogID,"'",""))&"'"
			...
			Set objCrypt = New CryptAES
				UserKey		= objCrypt.Encode(Join(Array(UserNo, Level, Userid, "home"), vbCrLf), SITENAME)
				secureKey   = objCrypt.Encode(Session.sessionID, SITENAME)
			Set objCrypt = Nothing
			UserKind =  BD_SELECT_Value("tbl_UserLog", "count(*)", "where UserNo="&UserNo&" and UserIP=''"&getIp&"'' and member_gubun=''home''")
			Response.Cookies("AUTH_USERKEY") = UserKey
			If UserKind > 0 Then
				valueDate = "loging = 0, intranet=0"
				whereSQl = " where UserNo="&UserNo&" and UserIP <> '"& getIp &"' and member_gubun='home'"
				...
				Call DB.execute(strSql, 1)
			Else
				log_flds		= "UserNo,  UserKey, UserIP, intranet, visitCount, RegDate, loging, member_gubun"
				log_values		= UserNo &",'"&secureKey&"','"&getIp&"',"&logInet&",1,'"&now()&"', 1, 'home'"
