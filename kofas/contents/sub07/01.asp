...
<% arng = "INS"
Call daum_ZipAddr()
Sql = "select id, name ,phone ,sch_gubun ,birthDay ,gubun ,state ,zipcode ,addr1 ,addr2, email, seq from group_member where id = '"&MYID&"' and gubun='청소년 조직'"
	...
	<div class="innerWrap">
		<div class="section1-cont1">
		 <form name="form1" method="post" action="/includefiles/global/youth_member_prog.asp" onsubmit="return send_it(this)">
			 ...
			 <TABLE class="table table-bordered mgt20" summary="청소년 조직화 회원가입 정보 입력창">
					<TBODY>
					<TR>
						<th style="width:25%"><label for="name">이름</TD>
						<TD><INPUT type="text" maxLength="10" value="<%=name%>" name="name" class="form-control" placeholder="이름" style="width:50%;" <%=readonly%>/></TD>
					</TR>
					<TR height="25">
					...
					<TR height="25">
						...
						<TD><div class="btn-group pull-left" data-toggle="buttons">
								<label class="btn btn-default <%If sch_gubun = "초" Then RW("active")%> ">
								 ...
								 <label class="btn btn-default  <%If sch_gubun = "고" Or sch_gubun = "" Then RW("active")%> ">
									<input type="radio" name="sch_gubun" value="고" <%If sch_gubun = "고" Or sch_gubun = "" Then RW("checked")%>> 고등학생
								 </label>
							 </div>
						</TD>
					 </TR>
				...
				<input type="hidden" name="memGubun" value="청소년 조직" />
