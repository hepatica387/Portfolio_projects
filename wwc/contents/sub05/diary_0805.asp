...
<div id="sub_content" class="sub0605">
	<div class="P01 center mgb5">
        <a href="javascript:;" onclick="$('#body-cont').load('<%=self_page%>?cur_Year=<%=preYear-1%>&cur_Month=<%=cur_Month%>')" class="btn btn-default " title="이전해"><i class="el el-backward"><span class="ir">이전해</span></i></a> &nbsp;
        <a href="javascript:;" onclick="$('#body-cont').load('<%=self_page%>?cur_Year=<%=preYear%>&cur_Month=<%=preMonth%>')" class="btn btn-default " title="이전달"><i class="el el-caret-left"><span class="ir">이전달</span></i></a> &nbsp;
        <span class="title3 "><%=cur_Year%>년 <%=cur_Month%>월</span>
        <a href="javascript:;" onclick="$('#body-cont').load('<%=self_page%>?cur_Year=<%=nextYear%>&cur_Month=<%=nextMonth%>');" class="btn btn-default mgl10" title="다음달"><i class="el el-caret-right"><span class="ir">다음달</span></i></a> &nbsp;
        <a href="javascript:;" onclick="$('#body-cont').load('<%=self_page%>?cur_Year=<%=nextYear+1%>&cur_Month=<%=cur_Month%>');" class="btn btn-default " title="다음해"><i class="el el-forward"><span class="ir">다음해</span></i></a>
    </div>
    <table class="table-bordered cols-100">
						...
						<div class="cols-100 left">
						 <% if not isnull(arrFlds) then
                             for n = 0 to ubound(arrFlds,2)
                                ...
                                RWBR("<div tabindex=""0"" role=""button"" data-toggle=""popover"" title="""&subject&""" data-content="""&content&" "&left(sTime,len(sTime)-3)&""" class=""hand EllipsText title09 pdl5 cols-100""> · "&subject&"</div>")
                            next
                          end if%>
						</div>
                </td>
 ...
 <div class="popover" role="tooltip"><div class="popover-title bold"></div><div class="arrow"></div><div class="popover-content"></div></div>
 <script>
    $(function () {
